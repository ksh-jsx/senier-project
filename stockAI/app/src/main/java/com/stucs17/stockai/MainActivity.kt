...
class MainActivity : AppCompatActivity(), IExpertInitListener, IExpertLoginListener, ITranDataListener {
    ...
    /*여기부터 로그인 작업*/
        ...
        override fun onLoginResult(isSuccess: Boolean, strErrorMsg: String?) {
            if(isSuccess == true)
                Toast.makeText(baseContext, "아이디,비밀번호 일치", Toast.LENGTH_SHORT).show()
            else
